...
## 費用を抑え、現場に無理なく取り入れられる改善・自動化の検討

- 業務名: 現在の業務フローを踏まえた、身近なツールによる負担軽減
- よくある困りごと: 大きな費用や運用変更を避けながら、Excel、フォーム、共有フォルダなどで手作業を減らす方法が分からない。
- 推奨Skill: `automation-planning-consultant`
- 補助Skill: `admin-workflow-consultant`
- 入力例: 作業頻度、件数、使用ツール、例外の多さ
- 出力例: 現状フロー、費用・現場負担・効果の比較、無理なく試せる改善案、相談文
- 注意点: システム変更は担当者へ確認する。
- 対象外: 医療安全に直接影響する自動処理
